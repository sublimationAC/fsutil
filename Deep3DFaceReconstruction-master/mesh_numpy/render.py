...
def get_block_idx(vertices, triangles, colors, h, w, c = 3, BG = None):
    ''' render mesh with colors
    Args:
        vertices: [nver, 3]
        triangles: [ntri, 3] 
    ...
    image = np.zeros((h, w, c), dtype = np.int)
    ...
    for i in range(triangles.shape[0]):
        ...
        for u in range(umin, umax+1):
            for v in range(vmin, vmax+1):
                ...
                if point_depth > depth_buffer[v, u]:
                    depth_buffer[v, u] = point_depth
                    image[v, u, :] = np.array([tri[0] ,tri[1],tri[2]])
    

    
    ans=np.zeros((vertices.shape[0]), dtype = np.bool)
    ans[image[image>0]]=1
    return ans